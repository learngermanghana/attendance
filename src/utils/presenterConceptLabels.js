...
export function presenterConceptLabel(value, { fallback = "Lesson concept" } = {}) {
  if (value && typeof value === "object" && clean(value.conceptLabel)) return clean(value.conceptLabel);
  const text = textFrom(value);
  if (!text) return clean(fallback) || "Lesson concept";
  const match = RULES.find((rule) => rule.test(text));
  return match?.label || clean(fallback) || "Lesson concept";
}

export function reviewConceptLabels(responses = [], options = {}) {
